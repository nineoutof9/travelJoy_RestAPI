...
public interface PlanProgress3Repository extends JpaRepository<PlanProgress3, Long> {
    ...
    List<PlanProgress3> findByPlanId(Long plan_Id);
    ...
    List<PlanProgress3> findByMinimumCost(Long minimum_Cost);

    // 특정 최대 비용에 해당하는 PlanProgress3 엔티티를 조회하는 메서드
    List<PlanProgress3> findByMaximumCost(Long maximum_Cost);

    // 특정 최소 평가에 해당하는 PlanProgress3 엔티티를 조회하는 메서드
    List<PlanProgress3> findByMinimumRate(Integer minimum_Rate);

    // 특정 최대 평가에 해당하는 PlanProgress3 엔티티를 조회하는 메서드
    List<PlanProgress3> findByMaximumRate(Integer maximum_Rate);
